...
import { API_URL, API_KEY, PAGE_SELECTOR, RECOMMENDATIONS } from '../../constants/api';
...
export class MovieDetailsService{
    ...
    getRecommended(id:number): Observable<MoviePage>{
        return this.http.get<MoviePage>(`${API_URL}/${id}${RECOMMENDATIONS}${API_KEY}`);
    }

    getPageRecommended(pageNumber:number, id: number ): Observable<MoviePage>{
        return this.http.get<MoviePage>(`${API_URL}/${id}${RECOMMENDATIONS}${API_KEY}${PAGE_SELECTOR}${pageNumber}`);
    }
